**To insert tables in Google Docs**:

1. Click **Insert** in the menu. 

![Image title](../image/insert.png)

2. Click <img src="/image/table_icon.png" class="emoji-icon"> **Table** to choose the necessary options for your table:

![Image title](../image/table.png)

- Click **Building blocks** to choose the necessary layout in the list.

![Image title](../image/table1.png)

- Click the necessary number of rows and columns for your table. 

![Image title](../image/table2.png)

Your table is inserted into the document. 


